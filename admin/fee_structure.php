...
// Get all fee structures grouped by course
...
foreach ($feeStructures as $fee) {
    ...
    if (!isset($groupedFees[$courseId])) {
        $groupedFees[$courseId] = [
            'course_name' => $fee['course_name'],
            'fees' => []
        ];
    }
    $groupedFees[$courseId]['fees'][] = $fee;
}
?>
    ...
    <div class="min-h-screen flex flex-col">
        ...
        <main class="flex-grow container mx-auto px-4 py-8">
            <div class="bg-white rounded-lg shadow-md p-6">
                <h1 class="text-2xl font-bold mb-6 text-center">Fee Structure Management</h1>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full bg-white">
                        ...
                        <tbody>
                            <?php foreach ($groupedFees as $courseId => $course): ?>
                                <tr class="border-b">
                                    <td class="py-3 px-4 text-center font-medium"><?= htmlspecialchars($course['course_name']) ?></td>
                                    <td class="py-3 px-4 text-center">
                                        <button onclick="toggleDetails(<?= $courseId ?>)" 
                                            class="text-blue-600 hover:text-blue-800 flex items-center justify-center mx-auto">
                                            <span>View Fees</span>
                                            <i class="fas fa-chevron-down ml-2 transition-transform duration-200" id="arrow-<?= $courseId ?>"></i>
                                        </button>
                                    </td>
                                </tr>
                                
                                <!-- Fee details row (hidden by default) -->
                                <tr id="details-<?= $courseId ?>" class="fee-details bg-gray-50">
                                    <td colspan="2" class="py-3 px-4">
                                        <table class="w-full">
                                            <thead>
                                                <tr class="bg-gray-100">
                                                    <th class="py-2 px-4 text-center">Type</th>
                                                    <th class="py-2 px-4 text-center">Period</th>
                                                    <th class="py-2 px-4 text-center">Total Fee</th>
                                                    <th class="py-2 px-4 text-center">Payment Option</th>
                                            ...
                                            <tbody>
                                                <?php foreach ($course['fees'] as $fee): ?>
                                                    <tr class="border-b">
                                                        <td class="py-2 px-4 text-center"><?= ucfirst($fee['fee_type']) ?></td>
                                                        <td class="py-2 px-4 text-center">
                                                            <?= $fee['fee_type'] === 'semester' ? 'Semester ' : 'Year ' ?><?= $fee['period'] ?>
                                                        </td>
                                                        <td class="py-2 px-4 text-center">NPR <?= number_format($fee['total_fee'], 2) ?></td>
                                                        <td class="py-2 px-4 text-center">
